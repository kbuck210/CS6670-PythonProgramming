...
# Part 1
# Modify quote search to allow multiple keywords and boolean operators
# If neither or both operators entered, assume AND condition
query=input("query:") 
...
for quote in data_list:
    count = 0
    for tok in tokens:
        found_at = quote.lower().find(tok)
        if found_at > 0:
            if orFlag:
                print("Found at " + str(line) + " " + quote)
                found = True
                break
            else:
                count += 1
...
if not found:
    print("Could not find the querey within the data")
# get end time
time2 = datetime.now()
